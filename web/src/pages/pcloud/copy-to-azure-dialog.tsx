...
import { useStartPCloudToAzure } from '@/api/process.api';
import {
...
export function CopyToAzureDialog({
  ...
  const auth = useAuth();
  const startCopy = useStartPCloudToAzure();

  const [azureDest, setAzureDest] = useState<AzureDestination>({ accountName: '', containerName: '', destinationFolder: '' });
  const [preparing, setPreparing] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  const canStart = azureDest.accountName && azureDest.containerName && (selectedFiles.length > 0 || selectedFolders.length > 0);
  ...
  const handleStart = useCallback(async () => {
    ...
    canStart, selectedFiles, selectedFolders, azureDest,
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-2">
          <AzureHierarchyPicker
            destinationPlaceholder="e.g. backups/pcloud"
