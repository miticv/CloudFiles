...
namespace CloudFiles.GooglePhotosToPCloud
{
    ...
    {
        [Function(Constants.ProcessGooglePhotosToPCloud_Start)]
        public static async Task<IActionResult> GooglePhotosToPCloudStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GooglePhotosToPCloud/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GooglePhotosToPCloudStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GooglePhotosToPCloudRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GooglePhotosToPCloudOrchestrator, request).ConfigureAwait(false);
