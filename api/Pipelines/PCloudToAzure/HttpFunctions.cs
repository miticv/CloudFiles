...
namespace CloudFiles.PCloudToAzure
{
    ...
    {
        [Function(Constants.ProcessPCloudToAzure_Start)]
        public static async Task<IActionResult> PCloudToAzureStorageStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/PCloudToAzure/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(PCloudToAzureStorageStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<PCloudToAzureRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.PCloudToAzureOrchestrator, request).ConfigureAwait(false);
