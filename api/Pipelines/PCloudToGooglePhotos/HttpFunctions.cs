...
namespace CloudFiles.PCloudToGooglePhotos
{
    ...
    {
        [Function(Constants.ProcessPCloudToGooglePhotos_Start)]
        public static async Task<IActionResult> PCloudToGooglePhotosStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/PCloudToGooglePhotos/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(PCloudToGooglePhotosStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<PCloudToGooglePhotosRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.PCloudToGooglePhotosOrchestrator, request).ConfigureAwait(false);
