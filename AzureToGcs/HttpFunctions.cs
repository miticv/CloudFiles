...
namespace CloudFiles.AzureToGcs
{
    ...
    {
        [Function(Constants.ProcessAzureToGcs_Start)]
        public static async Task<IActionResult> AzureToGcsStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/AzureToGcs/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(AzureToGcsStart));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<AzureToGcsRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.AzureToGcsOrchestrator, request).ConfigureAwait(false);
