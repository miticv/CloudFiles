...
namespace CloudFiles.GoogleDriveToGooglePhotos
{
    ...
    {
        [Function(Constants.ProcessGoogleDriveToGooglePhotos_Start)]
        public static async Task<IActionResult> GoogleDriveToGooglePhotosStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GoogleDriveToGooglePhotos/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GoogleDriveToGooglePhotosStart));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GoogleDriveToGooglePhotosRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GoogleDriveToGooglePhotosOrchestrator, request).ConfigureAwait(false);
