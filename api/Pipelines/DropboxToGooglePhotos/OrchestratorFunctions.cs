...
namespace CloudFiles.DropboxToGooglePhotos
{
    ...
    {
        [Function(Constants.DropboxToGooglePhotosOrchestrator)]
        public static async Task<object> DropboxToGooglePhotosOrchestrator(
            [OrchestrationTrigger] TaskOrchestrationContext context)
        {
            ILogger log = context.CreateReplaySafeLogger<object>();

            var request = context.GetInput<DropboxToGooglePhotosRequest>()!;

            log.LogInformation($"{Constants.DropboxToGooglePhotosOrchestrator}: Preparing request...");
            var preparedRequest = await context.CallActivityAsync<DropboxToGooglePhotosItemsPrepared>(
                Constants.DropboxToGooglePhotosPrepareList, request);

            log.LogInformation($"{Constants.DropboxToGooglePhotosOrchestrator}: FanOut request to {Constants.CopyDropboxToGooglePhotosOrchestrator} ...");
            var results = await context.CallSubOrchestratorAsync<NewMediaItemResultRoot>(
               Constants.CopyDropboxToGooglePhotosOrchestrator, preparedRequest);
        ...
        [Function(Constants.CopyDropboxToGooglePhotosOrchestrator)]
        public static async Task<NewMediaItemResultRoot> CopyDropboxToGooglePhotosOrchestrator(
            [OrchestrationTrigger] TaskOrchestrationContext context)
        {
            ILogger log = context.CreateReplaySafeLogger<object>();
            var itemsPrepared = context.GetInput<DropboxToGooglePhotosItemsPrepared>()!;
            ...
            for (int i = 0; i < total; i += batchSize)
            {
                ...
                var uploadTasks = chunk.Select(item =>
                    SafeUploadDropbox(context, item, retryOptions)).ToList();
        ...
        private static async Task<DropboxToGooglePhotosItemPrepared> SafeUploadDropbox(
            TaskOrchestrationContext context, DropboxToGooglePhotosItemPrepared item, TaskOptions options)
        {
            try
            {
                return await context.CallActivityAsync<DropboxToGooglePhotosItemPrepared>(
                    Constants.UploadDropboxFileToGooglePhotos, item, options);
            }
            catch (TaskFailedException)
            {
                item.StatusMessage = $"{item.DropboxPath}: Upload failed after retries.";
                return item;
            }
        }
    }
