...
namespace CloudFiles.GoogleDriveToAzure
{
    ...
    {
        [Function(Constants.ProcessGoogleDriveToAzure_Start)]
        public static async Task<IActionResult> GoogleDriveToAzureStorage(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GoogleDriveToAzure/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GoogleDriveToAzureStorage));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GoogleDriveToAzureRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GoogleDriveToAzureOrchestrator, request).ConfigureAwait(false);
