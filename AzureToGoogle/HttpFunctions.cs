...
namespace CloudFiles.AzureToGoogle
{
    public static class HttpFunctions
    {
        ...
        [Function(Constants.ProcessAzureToGooglePhotos_Start)]
        public static async Task<IActionResult> AzureStorageToGooglePhotos(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/AzureStorageToGooglePhotos/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(AzureStorageToGooglePhotos));
            try
            {
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.AzureStorageToGooglePhotosOrchestrator, request).ConfigureAwait(false);
