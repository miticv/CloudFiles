...
namespace CloudFiles.GooglePhotosToGcs
{
    ...
    {
        [Function(Constants.ProcessGooglePhotosToGcs_Start)]
        public static async Task<IActionResult> GooglePhotosToGcs(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GooglePhotosToGcs/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GooglePhotosToGcs));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GooglePhotosToGcsRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GooglePhotosToGcsOrchestrator, request).ConfigureAwait(false);
