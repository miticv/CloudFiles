...
namespace CloudFiles.GoogleStorageToGoogleDrive
{
    ...
    {
        [Function(Constants.ProcessGcsToDrive_Start)]
        public static async Task<IActionResult> GoogleStorageToDriveStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GoogleStorageToDrive/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GoogleStorageToDriveStart));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GcsToDriveRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GcsToDriveOrchestrator, request).ConfigureAwait(false);
