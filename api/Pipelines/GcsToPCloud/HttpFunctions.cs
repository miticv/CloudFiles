...
namespace CloudFiles.GcsToPCloud
{
    ...
    {
        [Function(Constants.ProcessGcsToPCloud_Start)]
        public static async Task<IActionResult> GcsToPCloudStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GcsToPCloud/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GcsToPCloudStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GcsToPCloudRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GcsToPCloudOrchestrator, request).ConfigureAwait(false);
