...
namespace CloudFiles.GooglePhotosToGoogleDrive
{
    ...
    {
        [Function(Constants.ProcessGooglePhotosToGoogleDrive_Start)]
        public static async Task<IActionResult> GooglePhotosToGoogleDriveStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GooglePhotosToGoogleDrive/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GooglePhotosToGoogleDriveStart));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GooglePhotosToGoogleDriveRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GooglePhotosToGoogleDriveOrchestrator, request).ConfigureAwait(false);
