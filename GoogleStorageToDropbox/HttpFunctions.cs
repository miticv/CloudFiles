...
namespace CloudFiles.GoogleStorageToDropbox
{
    ...
    {
        [Function(Constants.ProcessGcsToDropbox_Start)]
        public static async Task<IActionResult> GoogleStorageToDropboxStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GoogleStorageToDropbox/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GoogleStorageToDropboxStart));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GcsToDropboxRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GcsToDropboxOrchestrator, request).ConfigureAwait(false);
