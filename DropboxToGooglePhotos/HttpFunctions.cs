...
namespace CloudFiles.DropboxToGooglePhotos
{
    ...
    {
        [Function(Constants.ProcessDropboxToGooglePhotos_Start)]
        public static async Task<IActionResult> DropboxToGooglePhotosStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/DropboxToGooglePhotos/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(DropboxToGooglePhotosStart));
            try
            {
                var dropboxToken = DropboxUtility.VerifyDropboxHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<DropboxToGooglePhotosRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.DropboxToGooglePhotosOrchestrator, request).ConfigureAwait(false);
