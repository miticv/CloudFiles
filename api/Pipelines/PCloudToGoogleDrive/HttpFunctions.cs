...
namespace CloudFiles.PCloudToGoogleDrive
{
    ...
    {
        [Function(Constants.ProcessPCloudToDrive_Start)]
        public static async Task<IActionResult> PCloudToDriveStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/PCloudToGoogleDrive/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(PCloudToDriveStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<PCloudToDriveRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.PCloudToDriveOrchestrator, request).ConfigureAwait(false);
