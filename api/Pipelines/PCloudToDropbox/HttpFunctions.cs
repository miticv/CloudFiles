...
namespace CloudFiles.PCloudToDropbox
{
    ...
    {
        [Function(Constants.ProcessPCloudToDropbox_Start)]
        public static async Task<IActionResult> PCloudToDropboxStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/PCloudToDropbox/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(PCloudToDropboxStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<PCloudToDropboxRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.PCloudToDropboxOrchestrator, request).ConfigureAwait(false);
