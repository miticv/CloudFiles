...
namespace CloudFiles.AzureToGoogleDrive
{
    ...
    {
        [Function(Constants.ProcessAzureToDrive_Start)]
        public static async Task<IActionResult> AzureToGoogleDriveStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/AzureToGoogleDrive/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(AzureToGoogleDriveStart));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<AzureToDriveRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.AzureToDriveOrchestrator, request).ConfigureAwait(false);
