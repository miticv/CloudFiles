...
namespace CloudFiles.GoogleDriveToPCloud
{
    ...
    {
        [Function(Constants.ProcessGoogleDriveToPCloud_Start)]
        public static async Task<IActionResult> GoogleDriveToPCloudStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GoogleDriveToPCloud/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GoogleDriveToPCloudStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GoogleDriveToPCloudRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GoogleDriveToPCloudOrchestrator, request).ConfigureAwait(false);
