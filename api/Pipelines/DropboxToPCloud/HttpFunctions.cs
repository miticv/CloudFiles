...
namespace CloudFiles.DropboxToPCloud
{
    ...
    {
        [Function(Constants.ProcessDropboxToPCloud_Start)]
        public static async Task<IActionResult> DropboxToPCloudStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/DropboxToPCloud/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(DropboxToPCloudStart));
            try
            {
                var (pcloudToken, pcloudHostname) = PCloudUtility.VerifyPCloudHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<DropboxToPCloudRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.DropboxToPCloudOrchestrator, request).ConfigureAwait(false);
