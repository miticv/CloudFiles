...
namespace CloudFiles.GoogleDriveToGcs
{
    ...
    {
        [Function(Constants.ProcessGoogleDriveToGcs_Start)]
        public static async Task<IActionResult> GoogleDriveToGcsStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/GoogleDriveToGcs/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(GoogleDriveToGcsStart));
            try
            {
                var accessToken = await GoogleUtility.VerifyGoogleHeaderTokenIsValid(req).ConfigureAwait(false);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<GoogleDriveToGcsRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.GoogleDriveToGcsOrchestrator, request).ConfigureAwait(false);
