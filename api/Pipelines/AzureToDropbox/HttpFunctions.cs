...
namespace CloudFiles.AzureToDropbox
{
    ...
    {
        [Function(Constants.ProcessAzureToDropbox_Start)]
        public static async Task<IActionResult> AzureToDropboxStart(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "process/AzureToDropbox/start")] HttpRequest req,
            [DurableClient] DurableTaskClient starter,
            FunctionContext executionContext)
        {
            var log = executionContext.GetLogger(nameof(AzureToDropboxStart));
            try
            {
                var dropboxToken = DropboxUtility.VerifyDropboxHeaderTokenIsValid(req);
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync().ConfigureAwait(false);
                var request = JsonConvert.DeserializeObject<AzureToDropboxRequest>(requestBody)!;
                ...
                string instanceId = await starter.ScheduleNewOrchestrationInstanceAsync(
                    Constants.AzureToDropboxOrchestrator, request).ConfigureAwait(false);
